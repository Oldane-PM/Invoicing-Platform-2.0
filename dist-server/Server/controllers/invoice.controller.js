...
import { generateInvoiceForSubmission, replaceInvoiceAfterSubmissionEdit, shouldDeleteOldStorageFile, isInvoiceStaleVersusSubmission, } from '../services/invoices/generateInvoiceForSubmission';
 ...
 */
export async function getOrCreateInvoice(req, res, next) {
    try {
        ...
        if (stale) {
            console.log('[invoice.controller] Invoice older than submission revision; regenerating for submission:', submissionId);
        }
        else {
 ...
 */
export async function replaceInvoiceAfterSubmissionEditHandler(req, res, next) {
    try {
