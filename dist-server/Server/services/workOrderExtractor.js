 ...
 */
export async function extractDetailsFromFile(fileBuffer, mimeType, fileName) {
...
Do not write any markdown code block backticks (like \`\`\`json), explanations, or extra text. Return only the raw JSON.`;
    ...
    if (isImage) {
        ...
        if (!extractedText.trim()) {
            throw new Error('The document contains no readable text. If this is a scanned PDF, please upload it as an image file (PNG/JPG).');
        }
        messages = [
            {
                role: 'system',
