 ...
 */
export function shouldDeleteOldStorageFile(oldPath, newPath) {
...
export function isInvoiceStaleVersusSubmission(row) {
    ...
    if (Number.isNaN(revisionMs) || Number.isNaN(generatedMs))
        return false;
 ...
 */
export async function generateInvoiceForSubmission(submissionId) {
    ...
    const fixedAmount = submission.monthly_rate !== null && submission.monthly_rate !== undefined
 ...
 */
export async function replaceInvoiceAfterSubmissionEdit(submissionId, actorUserId) {
    const supabase = getSupabaseAdmin();
