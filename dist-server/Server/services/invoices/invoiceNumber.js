 ...
 */
export function incrementInvoiceNumber(previous) {
 ...
 */
export async function getNextInvoiceNumberForContractor(contractorId) {
    const supabase = getSupabaseAdmin();
    ...
    const candidates = [];
    if (profile?.last_invoice_number)
        candidates.push(profile.last_invoice_number);
    for (const row of issued ?? []) {
        if (row.invoice_number)
            candidates.push(row.invoice_number);
    }
    ...
    const previous = candidates.reduce((best, current) => trailingSequence(current) > trailingSequence(best) ? current : best);
