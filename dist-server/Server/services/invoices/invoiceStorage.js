 ...
 */
export async function uploadInvoicePdf(pdfBuffer, contractorId, submissionId, invoiceNumber) {
 ...
 */
export async function getSignedInvoiceUrl(storagePath, expiresIn = SIGNED_URL_EXPIRY) {
    const supabase = getSupabaseAdmin();
    const { data, error } = await supabase.storage
        .from(INVOICES_BUCKET)
 ...
 */
export async function invoicePdfExists(storagePath) {
    const supabase = getSupabaseAdmin();
    const { data, error } = await supabase.storage
        .from(INVOICES_BUCKET)
 ...
 */
export async function deleteInvoicePdf(storagePath) {
    const supabase = getSupabaseAdmin();
