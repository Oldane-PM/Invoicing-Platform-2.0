...
export async function generateW8BenPdf(data) {
...
function drawDoNotUseSection(doc) {
    let y = doc.y;
...
// ─── Note ──────────────────────────────────────────────────────────
function drawNote(doc) {
    let y = doc.y;
    doc.font(FONTS.bold).fontSize(6.5).text('Note: ', LEFT, y, { continued: true });
    doc.font(FONTS.regular).text('If you are resident in a FATCA partner jurisdiction (that is, a Model 1 IGA jurisdiction with reciprocity), certain tax account information may be provided to your jurisdiction of residence.', { width: CONTENT_WIDTH - 30 });
    y = doc.y + 4;
    hline(doc, y);
    doc.y = y + 2;
}
// ─── Part I ────────────────────────────────────────────────────────
function drawPart1(doc, data) {
    let y = doc.y;
    ...
    doc.font(FONTS.regular).text('Permanent residence address (street, apt. or suite no., or rural route). Do not use a P.O. box or in-care-of address.', LEFT + 14, y + 2, { width: CONTENT_WIDTH - 20 });
    ...
    if (data.mailingCity)
        doc.fontSize(9).text(data.mailingCity, LEFT + 14, subY4 + 10);
    ...
    if (data.mailingCountry)
        doc.fontSize(9).text(data.mailingCountry, MID + 84, subY4 + 10);
    y += 38;
    // Row: Line 5
...
function drawPart2(doc, data) {
    ...
    doc.font(FONTS.bold).text('Special rates and conditions (if applicable—see instructions): The beneficial owner is claiming the provisions of Article and paragraph', LEFT + 18, y + 3, { width: CONTENT_WIDTH - 24 });
    if (data.specialRatesArticle) {
        doc.font(FONTS.regular).fontSize(8);
        doc.text(`${data.specialRatesArticle} of the treaty identified on line 9 above to claim a ${data.specialRatesPercent || '___'}% rate of withholding on (specify type of income): ${data.specialRatesIncomeType || '___'}`, LEFT + 18, y + 20, { width: CONTENT_WIDTH - 24 });
        doc.moveDown(0.3);
        doc.font(FONTS.regular).fontSize(7).text(`Explain the additional conditions in the Article and paragraph the beneficial owner meets to be eligible for the rate of withholding: ${data.specialRatesConditions || 'N/A'}`, LEFT + 18, undefined, { width: CONTENT_WIDTH - 24 });
    }
    else {
...
function drawPart3(doc, data) {
    ...
    doc.text('Furthermore, I authorize this form to be provided to any withholding agent that has control, receipt, or custody of the income of which I am the beneficial owner or any withholding agent that can disburse or make payments of the income of which I am the beneficial owner. ', LEFT, y, { width: CONTENT_WIDTH, continued: true });
    doc.font(FONTS.bold).text('I agree that I will submit a new form within 30 days if any certification made on this form becomes incorrect.', { width: CONTENT_WIDTH });
    y = doc.y + 8;
    // Checkbox
    doc.rect(LEFT + 80, y, 9, 9).stroke();
    doc.font(FONTS.bold).fontSize(8).text('✓', LEFT + 82, y + 1);
    doc.font(FONTS.regular).fontSize(7).text('I certify that I have the capacity to sign for the person identified on line 1 of this form.', LEFT + 95, y + 1);
    y += 18;
    hline(doc, y);
    // ── Signature block ──
    const sigBlockH = 52;
    ...
    doc.text(`Digitally signed: ${format(data.signatureDate, 'yyyy-MM-dd HH:mm:ss')} UTC` +
        (data.ipAddress ? ` | IP: ${data.ipAddress}` : ''), LEFT, y);
    doc.y = y + 12;
}
// ─── Footer ────────────────────────────────────────────────────────
