-- Migration 044: Fix Notification Trigger Logic
-- Fixes unassigned OLD record on INSERT, removes reliance on missing period_start,
-- and ensures PAID notifications trigger even when status doesn't change but paid_at does.
...
BEGIN
  ...
  BEGIN
    ...
    -- Triggered when paid_at is set, even if status stays 'approved'
