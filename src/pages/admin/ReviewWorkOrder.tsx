...
import { useWorkOrder, useSignWorkOrderFinance } from "../../lib/hooks/useSystemWorkOrders";
import { WorkOrderDocument } from "../../components/shared/WorkOrderDocument";
import { SignaturePad } from "../../components/shared/SignaturePad";

interface ReviewWorkOrderProps {
  workOrderId: string;
  onClose: () => void;
}
...
export function ReviewWorkOrder({
  workOrderId,
  onClose,
}: ReviewWorkOrderProps) {
  const { data: workOrder, isLoading } = useWorkOrder(workOrderId);
  const signMutation = useSignWorkOrderFinance();
    ...
    <div className="space-y-6 flex flex-col h-full bg-gray-50/50 -mx-8 px-8 py-4 print:p-0 print:m-0 print:bg-white">
      <div className="flex justify-between items-center h-10 print:hidden">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Work Order Review</h2>
            <p className="text-sm text-gray-600">Review and counter-sign the contractor's work order.</p>
          </div>
        </div>
              ...
              {workOrder.status === "PENDING_FINANCE_SIGNATURE" ? (
                isSigning ? (
                  ...
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 w-full text-center">
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">Ready for Counter-Signature</h3>
                    <p className="text-gray-600 mb-6">
                      The contractor has signed this work order. Please review the document and apply your counter-signature to finalize it.
                    </p>
                    <Button 
                      size="lg"
                ...
                <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 w-full text-center">
                  <h3 className="text-lg font-semibold text-gray-900 mb-1">Awaiting Contractor</h3>
                  <p className="text-gray-600">This work order is currently awaiting the contractor's signature.</p>
                </div>
              )}
            </div>
          </div>
